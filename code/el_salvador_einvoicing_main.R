# =============================================================
# Title: The Effect of E-Invoicing on Tax Revenue and Compliance in El Salvador
# Author: Adam Lederman
# Description: Data prep, grouping, and event study estimation
# Version 1.0
# Last Updated: 8/27/2025
# Follow instructions on https://github.com/aelederman/el-salvador-einvoicing for replication
# =============================================================

# Load Packages
library("tidyverse") # readable data analysis
library("here") # for filepaths
library("lubridate") # for ymd
library("did") # for Callaway and Sant'Anna event study with multiple time periods
library("scales") # for commas on plot axes
library("broom") # cleans messy data frames for export

# === CODE LOOKUPS ===
# Load tax codes and standardize format for joining
tax_code_lookup <- read_delim(
  here("data","raw", "impuesto.csv"),
  delim = ";"
) %>%
  rename(TAX_CODE = CODIGO) %>%
  mutate(TAX_CODE = as.character(TAX_CODE),
         NOMBRE = as.character(toupper(NOMBRE)))

# Industry codes
industry_code_lookup <- read_delim(
  here("data","raw","actividadEconomica.csv"),
  delim = ";"
) %>%
  set_names(~ str_remove_all(.x, '"')) %>%
  rename(INDUSTRY_CODE = CODIGO, INDUSTRY_NAME = NOMBRE) %>%
  mutate(INDUSTRY_CODE = as.character(INDUSTRY_CODE)) %>%
  filter(str_length(INDUSTRY_CODE) == 2)

# VAT codes, returns from tax_code_lookup
vat_codes <- tax_code_lookup %>%
  filter(str_detect(NOMBRE, "IVA")) %>%
  pull(TAX_CODE)

# === DATA LOADER ===
# Data loading and cleaning function, returns standardized tibbles
# Strips whitespace and quotes, pads columns to equal widths, ensures variable types
load_and_clean <- function(path) {
  read_delim(path, delim = ";") %>%
    set_names(~ str_remove_all(.x, '"')) %>% # "~" is a temporary function applied to each column
    ...
    mutate(
      ...
      MUNICIPIO    = if_else(
        str_detect(MUNICIPIO, "^[0-9]{1,2}$"), # detects if MUNICIPIO is only 1-2 digits
...
# Loads files, creates row-appended VAT dataset ready for merging
raw_data <- map_dfr(files, load_and_clean) %>% 
  select(-any_of(c("ZONA","IMPORTANCIA","CLASE"))) %>% # removes unused admin columns
  filter(CODIMP %in% vat_codes) # filters on VAT 
...
firm_panel <- raw_data %>%
  ...
  left_join(industry_code_lookup, by = c("code2d" = "INDUSTRY_CODE")) %>%
  mutate(industry = coalesce(INDUSTRY_NAME, "Other"))

# === BUILD PANEL ===
# Collapses multiple rows (different tax payments) per firm per month
panel_obs <- firm_panel %>%
  group_by(firm_id, ANIO, MES) %>%
  summarize(
    revenue0  = sum(VALOR, na.rm = TRUE), # revenue0 is revenue as reported
...
# Create balanced monthly panel with zero revenue for missing months
monthly_data <- expand_grid(firm_id = all_firms, year_month = all_months) %>%
  ...
  mutate(
    ...
    revenue     = revenue0 + 1, # necessary for log calculation
    Y           = log(revenue),
    date        = ymd(paste(ANIO, MES, 1, sep = "-"))
  ) %>%
  select(-MUNICIPIO_fm)
...
# C+S (2021) requires stable units
# Cutoff one month before treatment
...
# Calculates average revenue before treatment for later cohort assignment
...
# Loose grouping logic, includes post-baseline entrants in data visualization under "late_entry"
firm_panel <- firm_panel %>%
  left_join(first_obs, by = "firm_id") %>%
  left_join(firm_baseline, by = "firm_id") %>%
  mutate(
    size_group_loose = case_when(
      first_date  > baseline_end ~ "late_entry",
...
# Strict grouping logic, excludes post-baseline entrants for event study (safest)
strict_baseline <- panel_obs %>%
  filter(date <= baseline_end) %>%
  group_by(firm_id) %>%
  summarize(
    avg_rev_pre_strict = mean(revenue0, na.rm = TRUE) * 12, # annualized avg revenue pre-baseline
    .groups = "drop"
  )

firm_panel <- firm_panel %>%
  left_join(strict_baseline, by = "firm_id") %>%
  mutate(
    size_group_strict = case_when(
      is.na(avg_rev_pre_strict)    ~ NA_character_,  # drops post-baseline firms
      avg_rev_pre_strict > 1e6     ~ "large",
      avg_rev_pre_strict > 1e5     ~ "medium",
      TRUE                           ~ "small"
    )
  )

# === ASSIGN 10-GROUPS BASED ON PRE-TREATMENT REVENUE ===
# Group sizes from communication with Salvadoran Ministry of Finance
target_sizes <- tibble(
  group_id    = paste0("G", 1:10),
  target_size = c(233,239,249,345,657,701,704,4982,14286,15000)
)

# Arranging firms into groups by descending pre-treatment avg revenue
firm_groups <- firm_panel %>%
...
# === DEFINE GROUP TIMING ===
# Creates and merges tibble of e-invoicing rollout dates with firm panel
group_timing <- tibble(
  group_id      = paste0("G", 1:10),
  notified_date = ymd(c(
...
firm_panel <- firm_panel %>% left_join(firm_info, by = "firm_id")

# === MERGE INTO BALANCED PANEL AND JOIN GROUPS ===
monthly_data <- monthly_data %>%
  mutate(
    size_group_strict = firm_panel$size_group_strict[match(firm_id, firm_panel$firm_id)],
    size_group_loose  = firm_panel$size_group_loose[match(firm_id, firm_panel$firm_id)]
  ) %>%
...
rev_total <- monthly_data %>%
  group_by(date) %>%
  summarize(total_rev = sum(revenue0, na.rm = TRUE), .groups = "drop")
...
# To compare with official reports
total_2023_rev <- rev_total %>%
  filter(year(date) == 2023) %>%
  summarize(total_rev = sum(total_rev, na.rm = TRUE))
...
# Prepare data
monthly_data <- monthly_data %>%
  left_join(firm_panel %>% distinct(firm_id, industry), by = "firm_id")

# Make sure industry and group_id are joined into firm_panel
firm_counts <- firm_panel %>%
...
group_industry_data <- monthly_data %>%
  ...
  summarize(total_rev = sum(revenue0, na.rm = TRUE),
...
for (i in seq_len(nrow(top50_muns))) {
  ...
  df_mun_cl <- monthly_data %>%
    filter(!is.na(size_group_strict), MUNICIPIO == mun_code)
  att_cl <- att_gt(
    yname         = "Y",
    tname         = "period",
    idname        = "id",
